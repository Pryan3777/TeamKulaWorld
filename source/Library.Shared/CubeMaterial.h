...
namespace Rendering
{
	class CubeMaterial : public FieaGameEngine::Material
	{
		RTTI_DECLARATIONS(CubeMaterial, FieaGameEngine::Material)

	public:
		CubeMaterial(FieaGameEngine::Game& game, std::shared_ptr<FieaGameEngine::Texture2D> colormap, std::shared_ptr<FieaGameEngine::TextureCube> environmentMap);
		CubeMaterial(const CubeMaterial&) = default;
		CubeMaterial& operator=(const CubeMaterial&) = default;
		CubeMaterial(CubeMaterial&&) = default;
		CubeMaterial& operator=(CubeMaterial&&) = default;
		virtual ~CubeMaterial() = default;
