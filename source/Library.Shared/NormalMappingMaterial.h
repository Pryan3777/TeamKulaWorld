...
namespace Rendering
{
	class NormalMappingMaterial : public FieaGameEngine::Material
	{
		RTTI_DECLARATIONS(NormalMappingMaterial, FieaGameEngine::Material)

	public:
		NormalMappingMaterial(FieaGameEngine::Game& game, std::shared_ptr<FieaGameEngine::Texture2D> colorMap, std::shared_ptr<FieaGameEngine::Texture2D> normalMap);
		...
		std::shared_ptr<FieaGameEngine::Texture2D> NormalMap() const;
		void SetNormalMap(std::shared_ptr<FieaGameEngine::Texture2D> texture);
	...
	private:
		...
		std::shared_ptr<FieaGameEngine::Texture2D> mNormalMap;
